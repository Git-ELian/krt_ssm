...
import com.krt.common.exception.KrtUploadException;
...
public class Assert {
    ...
    public static void isBlank(String str, String message) {
        if (StringUtils.isBlank(str)) {
            throw new KrtException(message);
        }
    }
    ...
    public static void isExcel(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new KrtUploadException("上传excel不能为空");
        }
        String fileExt = FilenameUtils.getExtension(file.getOriginalFilename());
        if (!"xls".equalsIgnoreCase(fileExt) && !"xlsx".equalsIgnoreCase(fileExt)) {
            throw new KrtUploadException("文件类型不合法");
        }
    }
}
